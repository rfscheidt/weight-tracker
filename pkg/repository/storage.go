...
func (s *storage) FindAllUser() []api.User {
	...
	for rows.Next() {
		var user api.User
		rows.Scan(&user.ID, &user.Name, &user.Age, &user.Height, &user.Sex, &user.ActivityLevel, &user.Email, &user.WeightGoal)
